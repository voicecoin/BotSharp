...
namespace MWL.DocumentResolver
{
	...
	internal class TFIDFMeasure
	{
		...
		private IDictionary _wordsIndex=new Dictionary<string, string>() ;
		...
		private void GenerateTermFrequency()
		{
			...
			{								
				...
				while (enums.MoveNext())
				{
					...
					int wordFreq=int.Parse(enums.Value.ToString());
		...
		private IDictionary GetWordFrequency(string input)
		{
			...
			{
                int tmp = CountWords(word, words);
				result[word]=tmp.ToString();
			}
